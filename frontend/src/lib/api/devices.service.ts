import type { CreateDeviceDto, Device, DeviceAccess, DeviceAccessListItem, Envelope, UpdateDeviceDto } from '@/types/api'
...
export class DevicesService {
  ...
  async grantAccess(deviceId: string, userId: string): Promise<DeviceAccess> {
    try {
      const { data } = await this.http<DeviceAccess>(`/devices/${deviceId}/access`, {
        method: 'POST',
        body: { user_id: userId },
      } as BetterFetchOption)
  ...
  async listAccess(deviceId: string): Promise<DeviceAccessListItem[]> {
    try {
      const { data } = await this.http<DeviceAccessListItem[]>(`/devices/${deviceId}/access`, {
        method: 'GET',
      } as BetterFetchOption)
