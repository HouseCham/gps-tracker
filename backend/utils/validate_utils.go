...
func ValidateStruct(requestBody any, validator *validator.Validate) (response.HTTPResponse[[]domain.ValidatorError], error) {
	if err := validator.Struct(requestBody); err != nil {
		return response.HTTPResponse[[]domain.ValidatorError]{
			StatusCode: fiber.StatusBadRequest,
			Message:    "Invalid request body",
			Data:       config.GetValidatorErrorMessage(err),
		}, err
	}
	return response.HTTPResponse[[]domain.ValidatorError]{}, nil
}
