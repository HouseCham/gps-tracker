...
// AccessService encapsulates per-device access checks and grants. It is consumed
...
//
// The users repository is needed so the AccessService can validate that a target
...
func NewAccessService(repo Repository, usersRepo users.Repository) *AccessService {
	return &AccessService{repo: repo, users: usersRepo}
}

// GetRole returns the access role the user has on the device.
// Returns domain.ErrNotFound when the user has no access.
func (s *AccessService) GetRole(ctx context.Context, userID, deviceID uuid.UUID) (domain.AccessRole, error) {
...
func (s *AccessService) RequireRole(ctx context.Context, userID, deviceID uuid.UUID, min domain.AccessRole) error {
...
func (s *AccessService) GrantAccess(ctx context.Context, actorID, deviceID, targetUserID uuid.UUID) (domain.Grant, error) {
...
func (s *AccessService) RevokeAccess(ctx context.Context, actorID, deviceID, targetUserID uuid.UUID) error {
	if actorID == targetUserID {
...
// actor is expected to have been authorized upstream (the HTTP middleware
// asserts the actor is an owner).
func (s *AccessService) ListUsersForDevice(ctx context.Context, _, deviceID uuid.UUID) ([]domain.UserWithAccessOnDevice, error) {
